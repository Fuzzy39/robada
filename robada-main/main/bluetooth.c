...
void bluetooth_add_services(struct ble_gatt_svc_def* service_definitions)
{
    ...
    int error = ble_gatts_count_cfg(service_definitions);
    if (error) 
    {
    ...
    error = ble_gatts_add_svcs(service_definitions);
    if (error) 
    {
...
void gap_init()
{
    ...
    int error = ble_svc_gap_device_name_set(DEVICE_NAME); // Does this get sent in the announce packet? If so, where?
